...
AVBitStreamFilter *annexbfilter;
AVBSFContext *annexbctx;

AVCodecContext *codec_ctx;
AVCodec *codec;
AVPacket *annex_b_packet;
AVPacket *outpacket;

void onPacket(AVPacket *pkt, gchar *streamId, int pktType)
{

  // g_assert(pkt->size == out_size);
  ...
  if (g_hash_table_contains(hash_table, streamId))
  {
    ...
    if (ctx->pipeline_initialized)
    {
      ...
      int ret = av_bsf_send_packet(annexbctx, pkt );
      if (ret != 0 && ret == AVERROR(EAGAIN))
      {
        printf("packet still present in the filter buffer please use recive packe\n");
      }else{
        printf("some thing went wrong\n");
      }
        printf("-----init------\n");

      ret =av_bsf_receive_packet(annexbctx,annex_b_packet);
      if (ret != 0 && ret == AVERROR(EAGAIN)){
        printf("no mopre packet in the filter please send packet\n");
      }
      pkt = annex_b_packet;
...
{
  enum AVCodecID id = stream->videopar->codec_id;
  annexbfilter = av_bsf_get_by_name("h264_mp4toannexb");
  annex_b_packet = av_packet_alloc(); //remove from here and alloc only once
  if (av_bsf_alloc(annexbfilter, stream) != 0)
  {
  ...
  if (codec_ctx == NULL)
  {
    printf("cannot allocate ctx\n");
  }

  int ret = avcodec_parameters_to_context(annexbctx->par_in, stream->videopar);
  if (ret < 0)
  {
    printf("cannot copy input codec parameters for {}");
  }
  annexbctx->time_base_in = *stream->rational;

  ret = av_bsf_init(annexbctx);
  if (ret < 0)
  {
    printf("cannot init bit stream filter context for {}");
  }

  //  printf("going to segfault\n");
  //  if (avcodec_open2(codec_ctx, codec, NULL) < 0)
  //  {
  //    printf("failed to open codec");
  //  }
  //  printf("lesssssss gooooooooooo no segfaluts\n");
}
...
void init_rtsp_server()
{
  ...
  setenv("GST_DEBUG_FILE", "~/usama/loggggggggggggggggggggggg.log", 1);
...
void setStreamInfo(char *streamId, AVCodecParameters *codecPar, AVRational *rational, int stream_type) // yaha pe copy karna hai problem aasakta hai dealloc ka
{
  if (g_hash_table_contains(hash_table, streamId))
  {
    ...
    if (stream_type == PACKET_TYPE_VIDEO)
    {
      ...
      g_print("%d    -------------------ccccc", ctx->videopar->width);
      // exit(0);
       init_Codec(ctx);
    }
