                                 ...
                                 GstAudioChannelPosition *pos)
{
  ...
  if (!none_layout && !gst_audio_check_valid_channel_positions(pos, nchannels, FALSE))
  {
    GST_ERROR("Invalid channel layout %" G_GUINT64_FORMAT
              " - assuming NONE layout",
              channel_layout);
    none_layout = TRUE;
  }
                    ...
                    const char *fieldname, ...)
{
  ...
  else if (encode)
  {
    ...
    default:
    {
      if (codec && codec->supported_framerates && codec->supported_framerates[0].num != 0 && codec->supported_framerates[0].den != 0)
      {
        GValue va = {
            0,
        };
                    ...
                    const char *fieldname, ...)
{
  ...
  else if (encode)
  {
      ...
      while (*layouts)
      {
        ...
        if (gst_ffmpeg_channel_layout_to_gst(*layouts, nbits_set, pos))
        {
                                                  ...
                                                  &mask))
          {
            ...
            if (!(nbits_set == 1 && pos[0] == GST_AUDIO_CHANNEL_POSITION_MONO) && !(nbits_set == 2 && pos[0] == GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT && pos[1] == GST_AUDIO_CHANNEL_POSITION_FRONT_RIGHT))
    ...
    else if (codec && codec->supported_samplerates && codec->supported_samplerates[0])
    {
      GValue va = {
          0,
      };
...
void gst_ffmpeg_videoinfo_to_context(GstVideoInfo *info, AVCodecContext *context)
{
  gint i, bpp = 0;
...
void gst_ffmpeg_audioinfo_to_context(GstAudioInfo *info, AVCodecContext *context)
{
  const AVCodec *codec;
                                  ...
                                  enum AVMediaType codec_type, const GstCaps *caps, AVCodecContext *context)
{
  ...
  case AV_CODEC_ID_SPEEDHQ:
  {
    ...
    if (context && (variant = gst_structure_get_string(str, "variant")) && strlen(variant) == 4)
    {
...
gst_ffmpeg_formatid_to_caps(const gchar *format_name)
{
  ...
  {
    caps =
        gst_caps_from_string("application/x-3gp; video/quicktime; audio/x-m4a");
  }
